...
export default function Process() {
  return (
    <div className="max-w-3xl mx-auto px-6 py-20">
      {/* Header */}
      <section className="pb-12 border-b border-warm-100">
      ...
      <section className="py-16 border-b border-warm-100">
        ...
        <div className="space-y-16">
          {methods.map((phase) => (
            <div key={phase.phase}>
              <p className="text-xs font-semibold text-warm-900 uppercase tracking-widest mb-7">
                {phase.phase}
              </p>
              <div className="space-y-3">
                {phase.tools.map((tool) => (
                  <div key={tool.name} className="p-5 rounded-lg border border-warm-200 bg-warm-50">
            ...
            <div key={group.category}>
              ...
              <div className="flex flex-wrap gap-2">
                {group.items.map((item) => (
                  <span
                    key={item}
                    className="text-sm px-3 py-1 rounded-full bg-warm-100 text-warm-600"
                  >
                    {item}
                  </span>
                ))}
